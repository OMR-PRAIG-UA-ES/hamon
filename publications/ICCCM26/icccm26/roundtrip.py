...
def _hamonpy_version() -> str:
    """The version of the library that produced these numbers, or 'unknown'."""
    try:
        from importlib.metadata import version
        return version("hamonpy")
...
def to_report_dict(results: list[ExampleResult]) -> dict:
    ...
    The report names the `hamonpy` that produced it. Deliberately no timestamp and no
    commit: those change on every run and every commit, and the golden test compares
    this file against a fresh run, so either would make it permanently stale. The
    version changes only on release, which is exactly when the numbers may move.
    """
    return {
        "hamonpy_version": _hamonpy_version(),
